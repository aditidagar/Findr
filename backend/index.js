...
app.get('/fetchUsers', (req, res) => {
  DatabaseManager.fetchUsers({ email: req.query.email })
    ...
    .then((result) => {
      if (result.length === 0) {
        ...
        res
          .status(404)
          .send(
            '404: User with email ' + req.body.email + " couldn't be found"
          );
      }
      ...
      DatabaseManager.fetchUsers({ courses: { $in: crs_regexes } })
        .then(async (users) => {
          users = users.filter((value, index, arr) => {
            return !value['_id'].equals(user._id);
          });
...
app.get('/fetchChatData', (req, res) => {
  ...
  DatabaseManager.fetchUsers({ email: req.query.from })
    .then(async (users) => {
      const user = users[0];
      var chatFound = false;
      let chat = null;
...
app.get('/fetchChats', (req, res) => {
  DatabaseManager.fetchUsers({ email: req.query.email })
    .then(async (users) => {
      ...
      DatabaseManager.fetchUsers({ email: { $in: chat_emails } })
...
app.post('/updateCourses', urlEncodedParser, (req, res) => {
  DatabaseManager.fetchUsers({ email: req.body.email })
    .then((result) => {
      if (result.length === 0) {
        console.log(`No user with email ${req.body.email}`);
        res
...
app.post('/new-user', urlEncodedParser, (req, res) => {
  ...
  DatabaseManager.insertUser(requestData)
    .then(async (result) => {
...
app.post('/login', urlEncodedParser, (req, res) => {
  ...
  DatabaseManager.fetchUsers({ email: requestData.email })
    .then((users) => {
      if (users.length < 1) {
...
io.on('connection', (socket) => {
  ...
  socket.on('new msg', (msg) => {
    DatabaseManager.fetchUsers({ email: msg.from })
      .then(async (users) => {
        const user = users[0];
        let chat = null;
        ...
        if (!msgHandled) {
          ...
          DatabaseManager.insertChat({ chat })
            .then((result) => {
              ...
              DatabaseManager.fetchUsers({ email: msg.to })
                .then((res) => {
                  let user = res[0];
                  user.chats.push(result.ops[0]._id);
